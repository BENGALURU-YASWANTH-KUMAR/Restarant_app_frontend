      ...
      <div className="card shadow-lg p-4" style={{ maxWidth: '400px', width: '100%', borderRadius: '15px' }}>
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" className="btn btn-primary btn-lg w-100"
                  style={{ backgroundColor: '#4CAF50', borderRadius: '10px' }}>Login</button>
        </form>

        <p className="text-center mt-4">
          Don't have an account?{' '}
          <button onClick={handleSignUp} className="btn btn-link"
                  style={{ textDecoration: 'none', color: '#007BFF' }}>Sign Up</button>
        </p>
      </div>
    </div>
  );
